...
vector<ZG_Points> CalculateGradient(const vector<ZG_Points>& points,double sgm, double eps){
    ...
    for(int i=0; i<n-1; i++){
        for(int j=i+1; j<n; j++){
            ...
            // Calculate the derivative of the Lennard-Jones potential energy
...
vector<ZG_Points> Min(vector<ZG_Points> &point, double eps, double sgm,const double& con=1, double step = 0.5,int max_seps=1000){
    ...
    while (abs(delta_energy) > con) {
        ...
        double line_search_energy = energyrecal(line_search_coords,eps,sgm);  // Calculate new energy
        // If the energy decreases, accept the new coordinates
        if (line_search_energy < energy) {
            delta_energy = line_search_energy - energy;  // Calculate energy changes
